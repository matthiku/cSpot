...
<nav class="navbar navbar-toggleable-sm fixed-bottom bg-primary center p-0" id="present-navbar">
    ...
    <div class="collapse navbar-collapse" id="presentationBottomNavbar">
        ...
        <!-- 
            DropUP Menu "Show"
        -->
        <div class="btn-group dropup ml-1" id="jumplist">
        ...
        <span class="nav navbar-nav mx-auto">
            ...
            -->
            @if (Auth::user()->ownsPlan($item->plan_id))
                <div class="btn-group dropup hidden-xs-down ml-4">
        ...
        <!-- 
            Personal Notes 
        -->
        <div class="dropup hidden-xs-down{{ $item->itemNotes->where('user_id', Auth::user()->id)->first() ? ' open' : '' }}">
